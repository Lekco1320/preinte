...
#else
#ifndef _PREINTE_HPP_
#define _PREINTE_HPP_
...
#define STT str_tool
    ...
    public:
        ...
        inline static STD::streambuf*& _Stream_cns() {
            static STD::streambuf* __Stream_cns = STD::cout.rdbuf();
            return __Stream_cns;
        }

        // Here are print_formats for all built-in types.
#define _SET_BUILT_IN_FORMAT(_NAME_, _TYPE_, _WIDTH_, _LENGTH_, _DIGITS_, _ALIGN_) \
    inline static print_format& _NAME_() { \
        static print_format _##_NAME_ { #_TYPE_, _WIDTH_, _LENGTH_, _DIGITS_, print_format::ALIGNMENT::_ALIGN_ }; \
        return _##_NAME_; \
    }
   
        _SET_BUILT_IN_FORMAT(_Format_d,   int,                7,  7,  0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_u,   unsigned int,       7,  7,  0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_hd,  short,              6,  7,  0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_uhd, unsigned short,     6,  7,  0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_f,   float,              9,  9,  2, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_ld,  long,               11, 11, 0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_uld, unsigned long,      11, 11, 0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_cs,  char,               80, 1,  0, LEFT );
        _SET_BUILT_IN_FORMAT(_Format_lf,  double,             9,  9,  2, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_lld, long long,          11, 11, 0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_zd,  unsigned long long, 11, 11, 0, RIGHT);
        _SET_BUILT_IN_FORMAT(_Format_Lf,  long double,        11, 11, 2, RIGHT);
...
#undef _SET_BUILT_IN_FORMAT
        ...
        inline static void print_id(const size_t& _Start, const size_t& _End, const size_t& _Width);
...
#define _SET_TABULATION_CHAR(_VARIABLE_, _VALUE_) \
    static constexpr const char* _VARIABLE_ = _VALUE_;
...
public:
#define _CONST_CONVERT(_PRINTER_) const_cast<const char**>(&_PRINTER_)

#define _SET_TABULATION_CHAR_POINTER(_VARIABLE_, _TARGET_) \
    inline static const char**& _VARIABLE_() { \
        static const char** _##_VARIABLE_ = _CONST_CONVERT(_TARGET_); \
        return _##_VARIABLE_; \
    }
...
#undef _SET_TABULATION_CHAR_POINTER
        ...
        static void basic_style() {
            ...
            bottom_left() = bottom_spliter() = bottom_middle() = bottom_right() = right_corner()  = _CONST_CONVERT(BASIC_CORNER);
            ver_border()  = ver_border_B()   = _CONST_CONVERT(BASIC_VER_BORDER);
            hor_border()  = hor_border_B()   = _CONST_CONVERT(BASIC_HOR_BORDER);
        }

        // Turn the table style to the normal one.
        static void normal_style() {
            top_left()      = _CONST_CONVERT(TAB_TOP_LEFT),      top_spliter()    = _CONST_CONVERT(TAB_TOP_MIDDLE_B);
            top_middle()    = _CONST_CONVERT(TAB_TOP_MIDDLE),    top_right()      = _CONST_CONVERT(TAB_TOP_RIGHT);
            middle_left()   = _CONST_CONVERT(TAB_MIDDLE_LEFT),   middle_spliter() = _CONST_CONVERT(TAB_MIDDLE_MIDDLE_B);
            middle_middle() = _CONST_CONVERT(TAB_MIDDLE_MIDDLE), middle_right()   = _CONST_CONVERT(TAB_MIDDLE_RIGHT);
            bottom_left()   = _CONST_CONVERT(TAB_BOTTOM_LEFT),   bottom_spliter() = _CONST_CONVERT(TAB_BOTTOM_MIDDLE_B);
            bottom_middle() = _CONST_CONVERT(TAB_BOTTOM_MIDDLE), bottom_right()   = _CONST_CONVERT(TAB_BOTTOM_RIGHT);
            ver_border()    = _CONST_CONVERT(TAB_VER_BORDER),    ver_border_B()   = _CONST_CONVERT(TAB_VER_BORDER_B);
            hor_border()    = _CONST_CONVERT(TAB_HOR_BORDER),    hor_border_B()   = _CONST_CONVERT(TAB_HOR_BORDER_B);
            left_corner()   = _CONST_CONVERT(TAB_LEFT_CORNER),   middle_corner()  = _CONST_CONVERT(TAB_MIDDLE_CORNER);
            right_corner()  = _CONST_CONVERT(TAB_RIGHT_CORNER),  middle_left_B()  = _CONST_CONVERT(TAB_MIDDLE_LEFT_B);
        }

#undef _CONST_CONVERT
...
#define _RE_CONSTEXPR(_VARIABLE_) constexpr const char* TAP::_VARIABLE_;

    _RE_CONSTEXPR(TAB_TOP_LEFT)
    _RE_CONSTEXPR(TAB_TOP_MIDDLE)
    _RE_CONSTEXPR(TAB_TOP_MIDDLE_B)
    _RE_CONSTEXPR(TAB_TOP_RIGHT)
    _RE_CONSTEXPR(TAB_MIDDLE_LEFT)
    _RE_CONSTEXPR(TAB_MIDDLE_MIDDLE)
    _RE_CONSTEXPR(TAB_MIDDLE_MIDDLE_B)
    _RE_CONSTEXPR(TAB_MIDDLE_RIGHT)
    _RE_CONSTEXPR(TAB_BOTTOM_LEFT)
    _RE_CONSTEXPR(TAB_BOTTOM_MIDDLE)
    _RE_CONSTEXPR(TAB_BOTTOM_MIDDLE_B)
    _RE_CONSTEXPR(TAB_BOTTOM_RIGHT)
    _RE_CONSTEXPR(TAB_HOR_BORDER)
    _RE_CONSTEXPR(TAB_HOR_BORDER_B)
    _RE_CONSTEXPR(TAB_VER_BORDER)
    _RE_CONSTEXPR(TAB_VER_BORDER_B)
    _RE_CONSTEXPR(TAB_MIDDLE_LEFT_B)
    _RE_CONSTEXPR(TAB_LEFT_CORNER)
    _RE_CONSTEXPR(TAB_MIDDLE_CORNER)
    _RE_CONSTEXPR(TAB_RIGHT_CORNER)
    _RE_CONSTEXPR(BASIC_CORNER)
    _RE_CONSTEXPR(BASIC_HOR_BORDER)
    _RE_CONSTEXPR(BASIC_VER_BORDER)
...
#undef _RE_CONSTEXPR
    ...
    inline void print(char** const _Ptr, const size_t& _Len_Arr, const size_t& _Len_Str, const char* _Name) {
